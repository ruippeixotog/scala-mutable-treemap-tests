...
class TreeMap[A, B] private (tree: RB.Tree[A, B])(implicit val ordering: Ordering[A])
  ...
  override def clear() = tree.root = null
   ...
   *              bound.
   */
  protected class TreeMapView(from: Option[A], until: Option[A]) extends TreeMap[A, B](tree) {
    ...
    override def foreach[U](f: ((A, B)) => U) = iterator.foreach(f)
