...
object RedBlackTree {
  ...
  def fixAfterDelete[A, B](tree: Tree[A, B], node: Node[A, B], parent: Node[A, B]): Unit = {
    var x = node
    var xParent = parent
  ...
  def isValid[A: Ordering, B](tree: Tree[A, B]): Boolean =
    isValidBST(tree.root) && isBlack(tree.root) && blackHeight(tree.root) >= 0 && size(tree.root) == tree.size
  ...
  /**
   * If all paths to descendent leaves of this node have the same number of black nodes, returns that number. If not,
   * returns -1. `null` nodes count as black nodes.
   */
  private[this] def blackHeight[A, B](node: Node[A, B]): Int = {
    if (node eq null) 1
    else if (node.red && (isRed(node.left) || isRed(node.right))) -1
    else if ((node.left ne null) && (node.left.parent ne node)) -1
    else if ((node.right ne null) && (node.right.parent ne node)) -1
    else {
      val lh = blackHeight(node.left)
      if (lh == -1) -1
      else {
        val bh = blackHeight(node.right)

        if (lh != bh) -1
        else if (isRed(node)) lh else lh + 1
      }
    }
  }
}
